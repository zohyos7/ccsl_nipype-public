...
def preproc_func(subject_list,
    ...
    if response == 'yes' :
        ...
        print(' - anat_example: sub-{subject_id}/anat/sub-{subject_id}_T1w.nii.gz')
        print(' - func_example: sub-{subject_id}/func/sub-{subject_id}_task-{task_name}_run-*_bold.nii.gz')
    ...
    extract = Node(ExtractROI(t_min=dummy_scans, t_size=-1, output_type='NIFTI'),
					name="extract")
    ...
    slicetime = Node(SliceTiming(num_slices = len(slice_order),
                                    ...
                                    time_repetition = TR,
                                    time_acquisition = TR-(TR/len(slice_order))),
					name="slicetime")
